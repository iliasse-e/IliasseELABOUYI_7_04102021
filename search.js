import { availableListElements as allListElements, updtatedList } from "./main.js";
 ...
 */
export function updateResults(selectedTags) {
    
    let currentListElements = updtatedList();
   ...
   /* for (let i=0; i<currentListElements.length; i++) {
        clear(currentListElements[i].split(' ').join('-'))
    }*/
            ...
            if (isGeneralSearch) {
             /*   for (let i=0; i<getObjectKeywords(recipes[recipeResults[recipe]]).length; i++) {
                ...
                if (!getObjectKeywords(recipes[recipeResults[recipe]]).includes(lastTag)) { // si la recette n'inclue pas le dernier tag
                    //console.log(selectedTags[tag] + " n'est pas inclue dans " + getObjectKeywords(recipes[recipeResults[recipe]]))
    
                    clear(recipes[recipeResults[recipe]].id); // supprime la carte de recette
                    ...
                    //console.log(selectedTags[tag] + " est inclue dans " + getObjectKeywords(recipes[recipeResults[recipe]]))
   ...
   /* for (let i=0; availableListElements.length; i++) {
        if (currentListElements.includes(availableListElements[i])) {
            unavailableListElements.pop(availableListElements[i])
        }
    }*/
